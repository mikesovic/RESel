...
#this single digest script assumes no divergent Y adaptor, so all frags in size range are sequenced, as long as their ends
#match the selective adaptor used. Assumes just a single selective adaptor in the case the RE has ambiguous bases.
...
total.frags.in.range <- function(min.frag.size,
  ...
  #filter the fragments for those with both overhangs matching an adaptor
  ...
  if (side1_re_name != side2_re_name) {
    ...
    pdf(file = paste0(args[1], "_out/", side1_re_name,	"/", side1_re_name, "_", side2_re_name, "_", selectives[1], ":", selectives[2], "-frag2k.pdf"))		
  }
  ...
  for (x in unique(bed$chr)) {
    ...
    for (i in 0:48) {
      ...
      if (nrow(feat_pos_df) > 0) {
        feat_pos_df <- feat_pos_df %>% group_by(r = row_number()) %>% 
...
#add in RE_name and RE_side to cuts data
...
side1_re <- frags_all %>%
   select(RE1) %>%
   distinct() %>% unlist() %>% as.character()

side2_re <- side1_re

#now start filtering
#for fragments for which seq1 and seq2 will both match the adaptor overhang used 
#evaluate for each possible adaptor overhang (assumes only one selective adaptor is used for single digests, even if RE site has ambiguities)
...
#side1_re is the name of the enzyme on side 1

#RE1_selectives stores the seqs associated with the RE - could be 1 or more

#will record number of fragments in target size range and report
#if there's a bed file, it will be cbound to this df before writing it.
...
for (i in 1:length(RE1_selectives)) {
    ...
    master_df <- rbind(master_df, current_selectives_df)
}  
...
if (!is.null(bed)) {
  ...
  for (i in 1:length(RE1_selectives)) {
    #for (j in 1:length(RE2_selectives)) {
      ...
      overall_summary_master <- rbind(overall_summary_master, overall_summary)
